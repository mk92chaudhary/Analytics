...
inspect(rules[1:5]) # support- how many time bought together, confidence - having bought A, what is chance of buying B, lift - are they increasing sales of each other, if vaue >1, strong relationship, 1, no relation, -1, negative relationship i.e. decreasing each others sales
quality(rules) 
head(quality(rules))
options (digits=2)
inspect (rules[1:5])
